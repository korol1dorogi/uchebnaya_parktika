...
> **Порт 8081, а не 8080.** На моей машине крутится другой сервис на 8080 порту, освободить нет возможности
>
...
Секреты читаются из переменных окружения и в репозиторий не коммитятся. (см. `.env.example`):
...
> функции БД работают только в Docker.
...
`8082` (`8082:8081` в compose) — на хосте `8080`/`8081` заняты (другие контейнеры). Свободный хостовый порт можно поменять в `docker-compose.yml`.
...
Код менять не нужно: сервисы названы `db` и `hasher`, как в compose.
Инструкция по запуску — [k8s/README.md](k8s/README.md).

## Добавление нового роута

1. Создать контроллер в `controllers/`, унаследовав
   `drogon::HttpController<YourController>`.
2. Описать методы в блоке `METHOD_LIST_BEGIN ... METHOD_LIST_END`
   через `ADD_METHOD_TO(...)`.
3. Пересобрать проект — новый файл подхватится автоматически.
